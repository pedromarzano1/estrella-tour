...
export async function enviarConfirmacionReserva(datos: DatosReserva) {
  ...
  await sendMail({
    to: datos.email,
    subject: `Confirmación de reserva — ${datos.origen} → ${datos.destino}`,
    html: `
<!DOCTYPE html>
<html lang="es">
...
export async function enviarCancelacionReserva(datos: Omit<DatosReserva, "metodoPago" | "monto">) {
  ...
  await sendMail({
    to: datos.email,
    subject: `Cancelación de reserva — ${datos.origen} → ${datos.destino}`,
    html: `
<!DOCTYPE html>
<html lang="es">
  ...
  <div style="max-width:600px;margin:40px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
    ...
    <div style="padding:40px;">
      ...
      <div style="background:#fef2f2;border-radius:8px;padding:24px;margin:24px 0;border-left:4px solid #dc2626;">
        <table style="width:100%;border-collapse:collapse;">
          <tr><td style="padding:6px 0;color:#6b7280;">Viaje cancelado</td><td style="padding:6px 0;color:#111827;font-weight:600;">${datos.origen} → ${datos.destino}</td></tr>
  ...
  <div style="max-width:600px;margin:40px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
    ...
    <div style="padding:40px;">
      ...
      <div style="background:#f0f9ff;border-radius:8px;padding:20px;margin:24px 0;border-left:4px solid #0ea5e9;">
        <p style="margin:0 0 6px;color:#374151;"><strong>Tu email de acceso:</strong> ${datos.email}</p>
  ...
  <div style="max-width:600px;margin:40px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
    ...
    <div style="padding:40px;">
      ...
      <div style="background:#eff6ff;border-radius:8px;padding:20px;margin:24px 0;border-left:4px solid #0ea5e9;">
        <p style="margin:0 0 8px;color:#374151;"><strong>Tu email de acceso:</strong> ${datos.email}</p>
...
export async function enviarNotificacionAdminPagoRecibido(datos: {
  ...
  await sendMail({
    to: adminEmail,
    subject: `✅ Pago recibido — ${datos.nombrePasajero} — ${datos.origen} → ${datos.destino}`,
    html: `
<p>Pago aprobado por Mercado Pago.</p>
<ul>
  <li><strong>Pasajero:</strong> ${datos.nombrePasajero} (${datos.emailPasajero})</li>
  <li><strong>Viaje:</strong> ${datos.origen} → ${datos.destino}</li>
